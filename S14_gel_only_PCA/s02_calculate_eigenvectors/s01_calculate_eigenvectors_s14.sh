...
# Started: CC30Apr2019
# Last updated: CC3June2019

# Stop at runtime errors
set -e

# Start message
echo "Calculate eigenvectors for gel only dataset"
date
echo ""
...
scripts_folder="${base_folder}/scripts/s14_gel_only_PCA/s02_calculate_eigenvectors"
...
# Source VCF
source_vcf_folder="${base_folder}/data/S14_gel_only_PCA/s01_select_gel_samples"
source_vcf_file="${source_vcf_folder}/selected_gel_cases_filtered_b38_genotypes.vcf.gz"

# Outputted initial PLINK file set
plink_dataset_folder="${base_folder}/data/S14_gel_only_PCA/s02_calculate_eigenvectors/p01_vcf_to_plink"
rm -fr "${plink_dataset_folder}"
mkdir -p "${plink_dataset_folder}"
initial_plink_dataset="${plink_dataset_folder}/gel_only_99kv_8ks_b38"
...
# Progress report
echo "Imported gel-only VCF to PLINK"
...
# Output folder
output_data_folder="${base_folder}/data/S14_gel_only_PCA/s02_calculate_eigenvectors/p02_exclude_variants_in_LD"
rm -fr "${output_data_folder}"
mkdir -p "${output_data_folder}"

# Output files
pairphase_LD="${output_data_folder}/pairphase_LD"
LD_pruned_dataset="${output_data_folder}/gel_only_70kv_not_in_LD"
...
output_data_folder="${base_folder}/data/S14_gel_only_PCA/s02_calculate_eigenvectors/p03_thinn_and_pca"
rm -fr "${output_data_folder}"
mkdir -p "${output_data_folder}"
thinned_dataset="${output_data_folder}/gel_only_10kv"
output_PCs="${output_data_folder}/gel_only_10kv_100PCs"
